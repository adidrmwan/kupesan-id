...
class KebayaController extends Controller
{
    ...
    public function submitStep2(Request $request)
    {
        ...
        $biayaSewa = KebayaBiayaSewa::where('fk_package_id', $package->id)
                        ->where('kebaya_durasi_hari', $durasi_sewa)
                        ->first();
        
        $booking_price = $biayaSewa->kebaya_biaya_sewa;
        $biaya_dry_clean = $package->price_dryclean;
    ...
    public function submitStep3(Request $request)
    {
        ...
        $booking_price = $booking->booking_price * $quantity;
        $biaya_dry_clean = $booking->biaya_dry_clean * $quantity;
        $deposit = $booking->deposit * $quantity;
        $total = $booking_price + $biaya_dry_clean + $deposit;
    ...
    public function bookingFinishedOnline(Request $request)
    {
        ...
        return redirect()->route('kebaya.schedule');
    }
    ...
    {   
        ...
        $partner = DB::table('partner')
                    ...
                    ->select('*')
                    ->first();
        $package_id = KebayaBooking::where('booking_id', $booking_id)->select('package_id')->first();
        $package = KebayaProduct::where('id', $package_id->package_id)->first();
        ...
        $booking =  KebayaBooking::join('kebaya_product', 'kebaya_product.id', '=', 'kebaya_booking.package_id')
                            ...
                            ->get();
        $booking2 = KebayaBooking::where('booking_id', $booking_id)->first();
        ...
        return view('partner.kebaya.detail-booking', ['partner' => $partner, 'booking' => $booking, 'package' => $package], compact('flag_date'));
    }
    ...
    public function showFormDayOff()
    {
        ...
        return view('partner.kebaya.dayoff', ['partner' => $partner], compact('jam', 'disableDates'));        
