...
class KebayaBookingController extends Controller
{
    ...
    public function step1(Request $request) {
        ...
        return view('online-booking.kebaya.step1', ['package' => $package, 'pid' => $package_id, 'partner_id' => $partner_id], compact('package', 'partner', 'biayaSewa', 'provinsi', 'kota', 'kecamatan'));
    ...
    public function step2(Request $request) {
        ...
        $kota = Regencies::where('id', $partner->pr_kota)->first();
        $kecamatan = Districts::where('id', $partner->pr_kec)->first();
        $booking_check = KebayaCheck::join('kebaya_product', 'kebaya_product.id', '=', 'kebaya_booking_check.package_id')
                            ->where('kebaya_booking_check.package_id', $package_id)
                            ->where('kebaya_booking_check.kuantitas', '=', $package2->quantity)
        ...
        if(empty($request->booking_date)) {
            return view('online-booking.kebaya.step2', compact('package','package_id','partner_id','partner', 'pu', 'disableDates', 'biayaSewa', 'provinsi', 'kota', 'kecamatan'));
        }
    ...
    public function step2a(Request $request) 
    {
        ...
        return view('online-booking.kebaya.step2a', compact('package','package_id','partner_id','partner', 'pu', 'disableDates', 'durasiSewa', 'tanggalPenerimaan', 'booking_id', 'biayaSewa', 'provinsi', 'kota', 'kecamatan'));
    }
    ...
    public function submitStep2a(Request $request){
        ...
        $booking->durasi_sewa = $durasi_sewa;
        $booking->save();
    ...
    public function submitStep3(Request $request)
    {
        ...
        $biayaSewa = KebayaBiayaSewa::where('fk_package_id', $package2->id)
                        ->where('kebaya_durasi_hari', $booking->durasi_sewa)
                        ->first();
        ...
        $booking_price = $biayaSewa->kebaya_biaya_sewa * $quantity;
        $deposit = $package2->deposit * $quantity;
        $biaya_dry_clean = $package2->price_dryclean * $quantity;
        if($request->lokasiAmbil == 'partnerku') {
            $biaya_kirim = '0';
        } elseif($request->lokasiAmbil == 'userku') {
            $biaya_kirim = '10000';
        }
        $total = $booking_price + $deposit + $biaya_dry_clean + $biaya_kirim;
        ...
        if(empty($booking->user_name)) {
            ...
            $booking->biaya_dry_clean = $biaya_dry_clean;
            $booking->biaya_kirim = $biaya_kirim;
    ...
    public function submitStep4(Request $request)
    {
        ...
        return view('online-booking.kebaya.step5', compact('partner', 'package', 'biayaSewa', 'provinsi', 'kota', 'kecamatan'));
    }
