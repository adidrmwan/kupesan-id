...
class BookingController extends Controller
{
    ...
    public function step2(Request $request) 
    {
        if (Auth::user()) {
            ...
            $booking_check = BookingCheck::join('ps_package', 'ps_package.id', '=', 'booking_check.package_id')
                                ->where('booking_check.package_id', $package_id)
                                ->where('booking_check.num_hour_1', '=', 1)
                                ->where('booking_check.num_hour_2', '=', 1)
                                ->where('booking_check.num_hour_3', '=', 1)
                                ->where('booking_check.num_hour_4', '=', 1)
                                ->where('booking_check.num_hour_5', '=', 1)
                                ->where('booking_check.num_hour_6', '=', 1)
                                ->where('booking_check.num_hour_7', '=', 1)
                                ->where('booking_check.num_hour_8', '=', 1)
                                ->where('booking_check.num_hour_9', '=', 1)
                                ->where('booking_check.num_hour_10', '=', 1)
                                ->where('booking_check.num_hour_11', '=', 1)
                                ->where('booking_check.num_hour_12', '=', 1)
                                ->where('booking_check.num_hour_13', '=', 1)
                                ->where('booking_check.num_hour_14', '=', 1)
                                ->where('booking_check.num_hour_15', '=', 1)
                                ->where('booking_check.num_hour_16', '=', 1)
                                ->where('booking_check.num_hour_17', '=', 1)
                                ->where('booking_check.num_hour_18', '=', 1)
                                ->where('booking_check.num_hour_19', '=', 1)
                                ->where('booking_check.num_hour_20', '=', 1)
                                ->where('booking_check.num_hour_21', '=', 1)
                                ->where('booking_check.num_hour_22', '=', 1)
                                ->where('booking_check.num_hour_23', '=', 1)
                                ->where('booking_check.num_hour_24', '=', 1)
                                ->select('booking_check.booking_date as disableDates')->get();
            $disableDates = array_column($booking_check->toArray(), 'disableDates');
        
            if(empty($request->booking_date)) {
                return view('online-booking.fotostudio.step2', ['package' => $package, 'pid' => $package_id, 'partner_id' => $partner->user_id], compact('partner', 'provinsi', 'kota', 'kecamatan', 'fasilitas', 'durasiPaket', 'tnc', 'disableDates'));
            }
        }
        ...
        if (empty($bookingcheck2)) {
            if (!empty($bookingcheck)) {
                ...
                if($notavailable == '1'){
                    ...
                    $booking->partner_id = $package_list->user_id;
    ...
    public function step5 (Request $request)
    {
        ...
        if ($booking->booking_status == 'belum_cek_ketersediaan' || $booking->booking_status == 'un_approved') {
            ...
            if($booking->booking_status != 'un_approved') {
                ...
                if ($booking->booking_status == 'belum_cek_ketersediaan') {
